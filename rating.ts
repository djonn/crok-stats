...
import { getDatePartFromDate, startOfDay, subtractDays } from "./dateUtils";
...
type RatingSystemType = "elo" | "openskill";
...
export function getPlayerRatingHistory<TRating>(
  ...
  const playerRatingHistory: Record<number, TRating> = {
    [startOfDay(dayBeforePlayersFirstMatch).getTime()]: system.defaultRating,
  };

  for (const match of matches) {
    ratings = getRatingsAfterMatch(ratings, match, system);
    if (hasPlayer(match, playerId)) {
      const dateOfMatch = match.createdAt;
      playerRatingHistory[startOfDay(dateOfMatch).getTime()] =
